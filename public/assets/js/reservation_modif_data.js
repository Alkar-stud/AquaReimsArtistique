document.addEventListener('DOMContentLoaded', function () {
    ...
    if (container) {
        ...
        if (donationSlider) {
            ...
            const storageKey = `donation_reservation_${reservationId}`;

            // Fonction pour mettre à jour l'affichage et le total
            const updateDonation = () => {
                const donationValue = parseFloat(donationSlider.value) || 0;
                const amountDue = parseFloat(amountDueEl.textContent.replace(',', '.')) || 0;

                // Mettre à jour l'affichage du montant du don
                donationAmountDisplay.textContent = donationValue.toFixed(2).replace('.', ',') + ' €';

                // Mettre à jour le total à régler
                const totalToPay = amountDue + donationValue;
                totalToPayWithDonationEl.textContent = totalToPay.toFixed(2).replace('.', ',') + ' €';

                // Afficher ou masquer la ligne du total avec don
                if (donationValue > 0) {
                ...
                localStorage.setItem(storageKey, donationValue);
            };

            // Restaurer la valeur au chargement de la page
            const savedDonation = localStorage.getItem(storageKey);
            if (savedDonation) {
                const savedValue = parseFloat(savedDonation);
                const maxValue = parseFloat(donationSlider.max);
                // S'assurer que la valeur sauvegardée ne dépasse pas le max actuel
                if (savedValue <= maxValue) {
                    donationSlider.value = savedValue;
                } else {
                    // Si elle dépasse (ex: le reste à payer a diminué), on la plafonne
                    donationSlider.value = maxValue;
                }
            }
            ...
            // Si le "reste à payer" est mis à jour par AJAX, il faut aussi mettre à jour le total avec don.
            // On utilise un MutationObserver pour détecter les changements sur l'élément "amount-due".
            ...
            observer.observe(amountDueEl, { childList: true, characterData: true, subtree: true });
        }
...
async function handlePayBalance(event) {
    ...
    const totalToPayInCents = amountDueInCents + donationInCents;
